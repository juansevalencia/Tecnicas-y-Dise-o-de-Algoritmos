...
const int INF = 1e9; // Aproximadamente infinito, pero depende del contexto
...
vector<tuple<int,int>> back_edges;
...
void Kruskal2(vector<tuple<int, int, int>>& edges, map<tuple<int,int,int>, string>& clasificacion_dic, int n) {
    ...
    for (int i = 0; i < edges.size(); i++) {
        ...
        if (peso_wi.size() > 1) {
            ...
            //no estoy ponienod las back edge en vacio devuelta o jo

            for(int iterator_dfs = 1 ; iterator_dfs <= n ; iterator_dfs++ ){ 
                if(estado[iterator_dfs] == 0 ){
                    dfs(iterator_dfs,-1);
                    raices.push_back(iterator_dfs);                   
                }
            }
            ...
            for(int iterator_puentes = 0; iterator_puentes < peso_wi.size() ; iterator_puentes++){
                if(!ya_clasifique_como_loop[i-iterator_puentes]){                                                                                 
                    ...
                    if(cubren(elem ,p ) == 0 && no_existe_igual( repu_chequear, repv_chequear ) && ( !pertenece({repu_chequear, repv_chequear}, back_edges) && !pertenece({repv_chequear, repu_chequear},back_edges) )){ //chequeo que no sea multiarista
