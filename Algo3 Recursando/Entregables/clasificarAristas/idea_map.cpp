...
void Kruskal2(vector<tuple<int, int, int>>& edges, map<tuple<int,int,int>, string>& clasificacion_dic, int n) {
    ...
    for (int i = 0; i < edges.size(); i++) {
        ...
        if (peso_wi.size() > 1) {
            ...
            vector<int> keys_to_erase2;
            for(auto& pair : tree_edges){
                keys_to_erase2.push_back(pair.first);
            }
            
            for(int key : keys_to_erase2){
                tree_edges.erase(key);
            }

            for(auto& pair :back_edges){
                back_edges_con_extremo_inferior_en[get<0>(pair)] = 0;
                back_edges_con_extremo_superior_en[get<1>(pair)] = 0;
            }
